<hr>
    ...
    <script>
        $('#document').ready(function(){
            var user= $("#user").val();
            var url = "{{route('plannings.user.pharmacies', [":id"])}}";
                 url = url.replace(':id', user);
            $.ajax({
                    url: url,
                    cache: false,
                    success: function(r) {
                        $("#displayPlanning").hide().html(r).fadeIn(100);
                    }
                });
        });
        ...
        $("#getDate").click(function() {
            ...
            if(de<a){
                var url = "{{route('plannings.recherche.pharmacies', [":de",":a",":id"])}}";
                 url = url.replace(':id', user);
                 url = url.replace(':a', a);
                ...
                $.ajax({
                    ...
                    cache: false,
                    success: function(r) {
                        $("#displayPlanning").hide().html(r).fadeIn(500);
                    }
                });
            }else{
